...
module.exports = {
            ...
            handler(ctx) {
                return this.database.collection.findOne({ email: ctx.params.email })
                    ...
                    .then(user => {
                        ...
                        let response = {
                            token: this.signedJWT({ type: "user_token", id: this.objectIDToString(user._id) }),
        ...
        resolveToken: {
            ...
            handler(ctx) {
                return new Promise((resolve, reject) => {
                    ...
                    .then(decoded => {
                        if (decoded.type == "user_token" && decoded.id) {
                            return this.database.collection.findOne({ _id: this.stringToObjectID(decoded.id) })
                                .then(user => {
                                    ...
                                    return { user: this.transformDocument(user, ctx.params.token) };
                                })
                                .catch(err => {
            ...
            handler(ctx) {
                return this.Promise.resolve()
                    ...
                    .then(async (user) => {
                        ...
                        let userId = this.objectIDToString(user._id); 
                        let token = this.signedJWT({ type: "reset_token", id: userId });
                        let data = {
                            email: user.email,
                            locale: user.locale,
                            token:  token
    ...
    methods: {
         ...
         */
        transformDocument (entity, token) {
            // normalize id
            entity.id = this.objectIDToString(entity._id);
            // hide internal fields
            delete entity.password;
            delete entity._id;
            if (token) entity.token = token;
            return entity;
        },
