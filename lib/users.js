...
/** Secret for JWT */
const JWT_SECRET = process.env.JWT_SECRET || "jwt-imicros-users-secret";
...
module.exports = {
    ...
    actions: {
            ...
            handler(ctx) {
                ...
                .then(decoded => {
                    if (decoded.type == "access_token" && decoded.id) {
                        return this.database.collection.findOne({ _id: this.stringToObjectID(decoded.id) }).then(user => {
        ...
        me: {
            handler(ctx) {
                ...
                .catch(err => {
                    if (err instanceof AuthError) throw err;
                    throw new AuthUserNotFound("user not created", { id: ctx.meta.user.id, err: err} );
                });
            }
        },
        ...
        requestPasswordResetMail: {
            ...
            handler(ctx) {
                ...
                .then(async (user) => {
                    if (!user)
                        return this.Promise.reject(new Error("user not found!"));
                ...
                .catch(err => {
                    return { error: "user not found", err: err };
                });
            }
        },
                ...
                .then(decoded => {
                    if (decoded.type == "reset_token" && decoded.id) {
                        ...
                        .then(res => {
                            ...
                            return { error: "user not found", id: decoded.id };
                        })
                        .catch(err => {
                            return { error: "user not found", err: err };
                        });
                    }
                    return this.Promise.reject(new Error("token not valid"));
                })
                .catch(err => {
                    return { error: "user not found or token not valid", err: err };
                });
            }
        },        
    ...
    created() {
        ...
        for (let property in this.calls) {
            if (this.calls.hasOwnProperty(property)) {
                if (this.settings[property] && this.settings[property].call) {
                    this.calls[property].call = this.settings[property].call;
                }
                if (this.settings[property] && this.settings[property].params) {
                    this.calls[property].params = this.settings[property].params;
                }
            }
        }
