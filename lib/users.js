...
module.exports = {
    ...
    settings: {
        ...
        /** Emit events instead service calls **/
        // emitEvents: true
            ...
            handler(ctx) {
                ...
                return this.database.collection.findOne({ email: ctx.params.email })
                    ...
                    .then(() => {
                        ...
                        if (this.disableVerification) user.verified = true;
            ...
            handler(ctx) {
                ...
                return this.database.collection.findOne({  _id: this.stringToObjectID(ctx.meta.user.id)  })
                    .then(async (user) => {
                        ...
                        if (this.emitEvents) {
                            ctx.meta.accessToken = this.accessToken;
    ...
    created() {
        ...
        this.disableVerification = _.get(this.settings, "disableVerification",false);
        this.emitEvents = _.get(this.settings, "emitEvents",false);
        
        // Settings for request of access token (for event handling in imicros-flow)
        this.services = {
            acl: _.get(this.settings, "services.acl",null)
        };
        this.serviceToken = process.env.SERVICE_TOKEN;
        this.groupId = process.env.FLOW_GROUP_ID;
        this.grantAccessToken = process.env.GRANT_ACCESS_TOKEN;
    ...
    async started() {
        ...
        // request accessToken
        if ( this.services.acl && this.serviceToken && this.groupId && this.grantAccessToken) {
            let params = {
                forGroupId: this.groupId
            };
            let opts = {
                meta: {
                    serviceToken: this.serviceToken,
                    grantAccessToken: this.grantAccessToken
                }
            };
            try {
                let res = await this.broker.call(this.services.acl + ".requestAccess",params,opts);
                if (res && res.token) {
